...
class Trainer:
    ...
    def __init__(self, model, model_config, train_dataset, train_dataset_len, test_dataset, test_dataset_len, config):
        ...
        with self.strategy.scope():
            ...
            self.optimizer = AdamWeightDecay(learning_rate=self.lr_cosine_decay if config.lr_decay else config.learning_rate, # type: ignore
        ...
        with self.strategy.scope():
            for epoch in epoch_bar:
                for inputs in progress_bar(self.train_dist_dataset,total=train_pb_max_len,parent=epoch_bar):
                    ...
                    self.tokens += tf.reduce_sum(tf.cast(labels >= 0, tf.int32)).numpy() # type: ignore
