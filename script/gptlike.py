...
def main():
  ...
  x_wall_valid, x_wall_test = train_test_split(
      x_wall_test, test_size=int(len(x_test) * 0.5)
  )
  ...
  inputs = layers.Input(Input(model_convex.input_shape[1 :]))
  model_convex = layers.Dense(1, activation="linear")(model_convex(input))
  model_cylinder = layers.Dense(1, activation="linear")(model_cylinder(input))
  model_wall = layers.Dense(1, activation="linear")(model_wall(input))
  concatenated = layers.concatenate([model_convex, model_cylinder, model_wall])
  softmaxed = layers.Dense(3, activation="softmax")(concatenated)
