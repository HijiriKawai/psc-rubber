...
plt.xlim([0,10]) # 軸範囲
plt.ylim([0,60])
plt.xticks([0, 2.5, 5, 7.5, 10]) # 軸に表示させる数値
...
for i in range(1,11):
    if i == 10:
        plt.plot(x,csv_convex[:,i:],c='tab:red',lw=2.5,label='445g')
    else:
        plt.plot(x,csv_convex[:,i:],c='tab:red',lw=2.5,)
for i in range(1,11):
    if i == 10:
        plt.plot(x,csv_cylinder[:,i:],c='tab:blue',lw=2.5,label='692g')
    else:
        plt.plot(x,csv_cylinder[:,i:],c='tab:blue',lw=2.5,)
for i in range(1,11):
    if i == 10:
        plt.plot(x,csv_wall[:,i:],c='tab:green',lw=2.5,label='1118g')
    else:
        plt.plot(x,csv_wall[:,i:],c='tab:green',lw=2.5,)
...
#%%
# グラフの保存
plt.savefig(f"{home}/data/plot_all.png")
